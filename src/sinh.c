...
static inline double always_inline __internal_sinh__(const double x)
{
  const double n = 0.5;
...
{
  return __internal_sinh__(x);
}

/**
 * @brief Compute the hyperbolic sine of a vector.
 * @param x An array of 64 bit floating-point values.
 * @param a A 64 bit floating-point value.
 * @param n The size of the array.
 */
void vec_sinh(double *x, const size_t n)
{
  for (uint64_t i = 0; i < n; i++)
  {
    x[i] = __internal_sinh__(x[i]);
  }
}

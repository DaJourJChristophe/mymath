...
static inline double always_inline __internal_cbrt__(const double x)
{
  const double n = 3.0;
  return root(x, n);
}
...
{
  return __internal_cbrt__(x);
}

/**
 * @brief Compute the cubed root of a vector.
 * @param x An array of 64 bit floating-point values.
 * @param a A 64 bit floating-point value.
 * @param n The size of the array.
 */
void vec_cbrt(double *x, const size_t n)
{
  for (uint64_t i = 0; i < n; i++)
  {
    x[i] = __internal_cbrt__(x[i]);
  }
}

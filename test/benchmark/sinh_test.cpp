...
static void BM_NaiveScalarSinhTest(benchmark::State& state)
{
...
static void BM_ScalarSinhTest(benchmark::State& state)
{
  const scalar_t epsilon = EPSILON;
  const scalar_t n       = MAX_TEST_DEPTH;
        scalar_t x       = MIN_TEST_DEPTH;
...
BENCHMARK(BM_NaiveScalarSinhTest);

BENCHMARK(BM_ScalarSinhTest);
